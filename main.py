import discord
import quiz_module
from quiz_module import Scores
...
async def on_ready():
    ...
    ayy_lmao = discord.utils.get(client.guilds, name="Big Booty Diva's server")
    for i in ayy_lmao.text_channels:
        print(f"Channels in BBD's Server: {i.name}")
...
async def on_message(message):
    ...
    if message.author != client.user and message.content.startswith("play") and client.game is False:
        ...
        quiz_module.play_song(message, client, song, client.game)
    ...
    if message.content.startswith("!num") and client.number_input is True:
        ...
        # play song
        quiz_module.play_song(message, client, client.imported_songs[client.counter], client.game)
...
client.run('NzA0NDczNTMwNDg1NzAyNzQ4.XqdqqA.wZOW9d3UDgNgxT-LfkAiTXCQ6ks')